logistic_fit_GR = function(input_data,groupingColumns) {
  ...
  for(i in 1:length(experiments)) {
    ...
    #===== unconstrained fit ==========
#     output_model_new = try(drm(GR~concentration, experiment, data=data_exp, fct=LL.3u(names=c('Hill','GRinf','EC50'))))
#     if(class(output_model_new)!="try-error") {
#       parameters[i,] = c(as.numeric(coef(output_model_new)))
#       # F-test for the significance of the sigmoidal fit
#       Npara = 3 # N of parameters in the growth curve
...
#       parameters[i,]=NA
#     }
    #rownames(parameters)[i] = experiments[i]
    #==================================
  ...
  parameters$GR_AUC = AOC
  parameters = parameters[,c('GR50','GRmax','GR_AUC','EC50','GRinf','Hill','log10(GR50)','log10(EC50)')]
  ...
  # Add specified values for flat fits: EC50 = 0, Hill = 0.01 and GR50 = +/- Inf
  for(i in 1:dim(parameters_show)[1]) {
    if(parameters_show$fit[i] == "flat") {
      parameters_show$EC50[i] = 0
  ...
  delete_cols = which(colnames(parameters) %in% c('log2(Hill)','log10(GR50)', 'log10(EC50)','fit','flat_fit'))
  parameters_show = parameters_show[,-delete_cols]
  parameters_show$GR50 = prettyNum(parameters_show$GR50, digits = 3)
  parameters_show$GRmax = prettyNum(parameters_show$GRmax, digits = 3)
  parameters_show$GR_AUC = prettyNum(parameters_show$GR_AUC, digits = 3)
  parameters_show$EC50 = prettyNum(parameters_show$EC50, digits = 3)
  parameters_show$GRinf = prettyNum(parameters_show$GRinf, digits = 3)
  parameters_show$Hill = prettyNum(parameters_show$Hill, digits = 3)
  parameters_show$r2 = prettyNum(parameters_show$r2, digits = 3)
  parameters_show$pval = prettyNum(parameters_show$pval, digits = 3)
